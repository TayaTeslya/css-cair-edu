...
localStorage.setItem("user", JSON.stringify({
    ...
    "isStaff": true      // Пользователь-администратор, пример true или false
}));
...
function changeCategory() {
    switch (selectedCategory) {
        ...
        case 3: // Избранное для ученика, на проверке для администратора
            if (userInfo.isStaff) {
                setLevels((element) => !element.checked);
            }
            else {
...
searchLevel.addEventListener('keyup', (event) => {
    let searchValue = searchLevel.value.trim().toLowerCase();
    if (searchValue) {
